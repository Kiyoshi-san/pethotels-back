...
import { isAdmin, isAuth, isSellerOrAdmin } from "../models/utils.js";
...
productRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (seller) {
      ...
      res.status(500).send({ message: "Nenhum vendedor encontrado" });
    }
  })
);
...
productRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const priceFilter = min && max ? { price: { $gte: min, $lte: max } } : {}; // $gte - greater than min; $lte - less than max
    ...
    // RETURN QUANTITY OF THE MATCHED PRODUCTS
...
productRouter.put(
  ...
  isAdmin,
  expressAsyncHandler(async (req, res) => {
    const product = Product.findById(res.params.id);
    if (product) {
